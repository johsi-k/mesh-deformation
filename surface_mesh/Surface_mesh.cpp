...
{
  if (!findices_)
      findices_ = face_property<Indices>("f:indices");
  ...
  for(Face_iterator fit=faces_begin(); fit!=fend; ++fit){
    ...
    int vid[3];
    vid[0] = from_vertex(h).idx();
    vid[1] = to_vertex(h).idx();
    vid[2] = to_vertex(next_halfedge(h)).idx();

    findices_[f] = Indices(vid[1], vid[2], vid[0]);
  }
}
